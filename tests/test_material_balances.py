...
class TestSolvers:
    """Test suite for solver functions."""

    def test_jam_calculation_basic(self):
        """Test basic jam calculation with default parameters."""
        result = calculate_stream_based_on_jam_mass(1000.0)

        assert "strawberry_mass" in result
        assert "sugar_mass" in result
        assert "water_mass" in result
        assert "jam_mass" in result

        assert result["jam_mass"] == 1000.0
        assert result["strawberry_mass"] > 0
        assert result["sugar_mass"] > 0
        assert result["water_mass"] > 0

    def test_jam_calculation_mass_balance(self):
        """Test that jam calculation satisfies mass balance."""
        result = calculate_stream_based_on_jam_mass(1000.0)

        # Input mass = strawberries + sugar
        input_mass = result["strawberry_mass"] + result["sugar_mass"]

        # Output mass = jam + evaporated water
        output_mass = result["jam_mass"] + result["water_mass"]

        # Should be approximately equal
        assert np.isclose(input_mass, output_mass, atol=1e-6)

    def test_jam_calculation_zero_mass(self):
        """Test jam calculation with zero jam production."""
        result = calculate_stream_based_on_jam_mass(0.0)

        assert result["jam_mass"] == 0.0
        assert result["strawberry_mass"] == 0.0
        assert result["sugar_mass"] == 0.0
        assert result["water_mass"] == 0.0

    def test_jam_calculation_different_ratio(self):
        """Test jam calculation with different strawberry:sugar ratio."""
        # Test with different k value
        result = calculate_stream_based_on_jam_mass(1000.0, k=1.0)  # 1:1 ratio

        assert result["jam_mass"] == 1000.0
        assert result["strawberry_mass"] > 0
        assert result["sugar_mass"] > 0

    def test_jam_calculation_different_water_contents(self):
        """Test jam calculation with different strawberry water content."""
        result_standard = calculate_stream_based_on_jam_mass(
            1000.0,
            strawberry_water_content=0.85
        )
        result_drier = calculate_stream_based_on_jam_mass(
            1000.0,
            strawberry_water_content=0.70
        )

        # Drier strawberries should require less input mass
        input_standard = (
            result_standard["strawberry_mass"] + result_standard["sugar_mass"]
        )
        input_drier = result_drier["strawberry_mass"] + result_drier["sugar_mass"]

        assert input_drier < input_standard

    def test_jam_calculation_different_jam_water_content(self):
        """Test jam calculation with different final jam water content."""
        result_wet = calculate_stream_based_on_jam_mass(
            1000.0,
            jam_water_content=0.4
        )
        result_dry = calculate_stream_based_on_jam_mass(
            1000.0,
            jam_water_content=0.2
        )

        # Drier jam should require more water evaporation
        assert result_dry["water_mass"] > result_wet["water_mass"]

    def test_jam_calculation_negative_mass_raises_error(self):
        """Test that negative jam mass raises ValueError."""
        with pytest.raises(ValueError, match="non-negative"):
            calculate_stream_based_on_jam_mass(-100.0)

    def test_jam_calculation_returns_dict(self):
        """Test that function returns proper dictionary structure."""
        result = calculate_stream_based_on_jam_mass(500.0)

        assert isinstance(result, dict)
        assert len(result) == 4
        for key in ["strawberry_mass", "sugar_mass", "water_mass", "jam_mass"]:
...
class TestIntegration:
    ...
    def test_jam_problem_in_process_unit(self):
        """Test using process unit framework for jam problem."""
        # Create components for jam problem
        solids = Component("Solids")
        water = Component("Water")

        # Create streams
        strawberry_stream = Stream(
            "strawberries", 2000.0, "mass",
            [solids, water],
            {"Solids": 0.15, "Water": None}
        )
        sugar_stream = Stream(
            "sugar", 2444.44, "mass",
            [solids, water],
            {"Solids": 1.0, "Water": None}
        )
        jam_stream = Stream(
            "jam", 1000.0, "mass",
            [solids, water],
            {"Solids": 2/3, "Water": None}
        )
        water_vapor_stream = Stream(
            "water_vapor", 1366.67, "mass",
            [solids, water],
            {"Solids": 0.0, "Water": 1.0}
        )

        # Create heater unit
        heater = ProcessUnit(
            "Heater",
            [strawberry_stream, sugar_stream],
            [jam_stream, water_vapor_stream]
        )

        # All flows are specified, so just verify balances
        heater.print_report()

    def test_evaporator_example_from_notebook(self):
        """Test the evaporator example from the original notebook."""
        acetone = Component("Acetone")
        water = Component("Water")
        ...
        n = Stream(
            "n", 10.0, "mole",
            [acetone, water],
            {"Acetone": 0.35, "Water": None}
        )
        vapor = Stream(
            "vapor", None, "mole",
            [acetone, water],
            {"Acetone": 0.75, "Water": None}
        )
        liquid = Stream(
            "liquid", None, "mole",
            [acetone, water],
            {"Acetone": 0.187, "Water": None}
        )

        evaporator = ProcessUnit("Evaporator", [n], [vapor, liquid])

        assert evaporator.is_solvable()
        evaporator.solve_material_balances()

        # Verify against expected results
        assert np.isclose(vapor.flow_rate, 2.8952, atol=0.001)
        assert np.isclose(liquid.flow_rate, 7.1048, atol=0.001)
